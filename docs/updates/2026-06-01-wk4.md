### Todo
- [ ] Improve the bad results we have currently
- [ ] Try solve 2d-poisson if have time
- [ ] Try seeing if other methods increase max m if required (such as Gradient Checkpointing)
...
- Try other architectures like ones under Operator Network Architectures (DeepONet or Hypernetworks) but we shall not do that for now.
- Lowering the Kaczmarz learning rate parameter
