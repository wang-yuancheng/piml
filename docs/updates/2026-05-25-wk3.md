...
Friday - Do the part labelled Possible Improvements for Later, 
...
JAX pushes `x_batch` and `y_batch` through your `FeatureExtractor` using the current hidden params. The GPU holds  $f_{xx}, f_{yy}$ in VRAM to assemble the block matrix $A_B$. The Kaczmarz projection runs, generating the new $w$. JAX instantly deletes the features and $A_B$ from VRAM.

Then, we want to update params with optax. JAX pushes `x_batch` and `y_batch` through the `FeatureExtractor` again. It builds a computational graph in VRAM, runs backpropagation and updates.
...
\right\|^2
=
1000\,\left\|A_{\text{bc}}w - b_{\text{bc}}\right\|^2
$$
