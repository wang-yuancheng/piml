...
- [x] Try replace direct solve with sketch-and-project 
...
* **Friday** - Replace direct solve with sketch-and-project 
* **Monday**  - Test random sampling of boundary points instead of adding all of it, fix some for loop problems and bugs.
* **Tuesday**  - Try data_T20_S30_G50_reduced dataset with RFF + MLP, rubbish results, still looking into this.
...
#### Using Sketch-And-Project
*For the below description, we are passing a batched amount of coordinates, say 1000, for each sample and not the full grid of spatial coordinates.*

With direct solve: $w_{k+1} = w_k - \alpha A_{block}^T (A_{block} A_{block}^T + \epsilon I)^{-1} (A_{block} w_k - b_{block})$ by chunk assembling $A^T A$. But what if $m$ is very large?

With kaczmarz: $w_{k+1} = w_k - \alpha A_{chunk}^T (A_{chunk} A_{chunk}^T + \epsilon I)^{-1} (A_{chunk} w_k - b_{chunk})$. We can achieve space complexity of $O(\max(N_{chunk}^2, m_{chunk} N_{chunk}))$ for $(A A^T + \epsilon I)^{-1} (A w_k - b)$, which essentially is $O(1)$. For full details, see [Week 4 Notes](../notes/wk4.md).

**Important:** We need to make sure Jax does not build a computational tree when we are solving for $w$ but only let it build the tree when we are trying to find the loss in the dot product. This is because in our setup, JAX calculates the MLP's second-order derivatives (the PDE physics) $16N$ times per batch ($8 \text{ samples} \times N \text{ sweeps} \times 2 \text{ chunk passes}$). By addinng `w_current = jax.lax.stop_gradient(w_current)`, we improved training speed by 20x.

##### Details
Direct solve with `total_features = 1024`: $A^T A$ is $1024 \times 1024$ matrix.
Sketch-and-project with sampled PDE points (1000) + bc points (about 508 for a 128x128 grid): $G$ is a 1508 $\times$ 1508 matrix.

This means right now, we are actually using more memory, but once $m$ increases a lot, it will be much more efficient. However, we need to also take note that we are putting all the boundary points in right now, so if we deal with a $1000 \times 1000$ grid, we would sample $4996$ points, meaning we likely need to random sample the boundary points too.
...
Each epoch takes about 11 mins.
...
#### Friday
1 Sweep: Each epoch takes about 20 mins
Epoch 001 | Train Loss: 3.7628e-02 | Val MSE: 2.0083e-05 | TikReg: 1.03e-05
Epoch 002 | Train Loss: 3.7580e-02 | Val MSE: 2.0155e-05 | TikReg: 1.03e-05
Epoch 003 | Train Loss: 3.7655e-02 | Val MSE: 2.0269e-05 | TikReg: 1.03e-05
<img src="../assets/wk5/friday-results.png" width="1000">

5 Sweeps: Each epoch takes about 20mins
Epoch 001 | Train Loss: 7.3484e-02 | Val MSE: 3.1181e-05 | TikReg: 1.03e-05

10 Sweeps: Each epoch takes about 22 mins
Epoch 001 | Train Loss: 1.8402e-01 | Val MSE: 6.9962e-05 | TikReg: 1.03e-05

20 Sweeps: Each epoch takes about 23 mins
Epoch 001 | Train Loss: 6.3125e-01 | Val MSE: 2.0786e-04 | TikReg: 1.03e-05

**It appears that 1 sweep and 1 epoch has the lowest Val MSE...**

#### Monday
Change to sampling of boundary condition maintains accuracy.

**Memory usage**
Idle:  ~1350MiB / 8192MiB
Direct Solve: ~2600MiB / 8192MiB
Sketch-and-Project: ~2300MiB / 8192MiB

#### Tuesday
Rubbish Results, still checking...